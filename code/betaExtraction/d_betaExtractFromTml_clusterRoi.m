...
for iSub = 1:length(subjectList)
    ...
    %where is the glm
    glmDir =char(fullfile(fileparts(mfilename('fullpath')),'..', '..','outputs','derivatives','bidspm-stats',char(subID),strcat('task-',taskName,'_space-IXI549Space_FWHM-6')));
    ...
    opt.maskDir = fullfile(fileparts(mfilename('fullpath')),'..', '..','outputs','derivatives','cluster-roi','subjectCluster_base2pt6',char(subID));
    maskDir = opt.maskDir;
