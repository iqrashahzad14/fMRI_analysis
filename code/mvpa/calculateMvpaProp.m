function accu = calculateMvpaProp(opt)
...
%   condLabelNb = [1 2 3 4 5 6 ]; [2 2 1 1 5 6]
  ...
  for iSub = 1:numel(opt.subjects)
    ...
    % using Glasser atlas for motor cortex and proprioceptive activity
%     voxelNb=opt.maskVoxelNb;radiusNb=4;
%     opt.maskPath = fullfile(fileparts(mfilename('fullpath')), '..','..', '..','code' ,'visTacMotion_ROI','src','inputsGlasser');    
%     % masks to decode/use
%     opt.maskName = {'space-MNI_label-glasserMotor_mask.nii',...
%     'space-MNI_label-galsserSomato_mask.nii',...
%     'space-MNI_label-glasserVip_mask.nii'};
...
%         };
    ...
    opt.maskPath = fullfile(fileparts(mfilename('fullpath')), '..','..', 'outputs','derivatives' ,'bidspm-roi','groupSphere');%,strcat('sub-',subID));
...
%         'space-MNI_label-glasserMotor_mask.nii',...
%         'space-MNI_label-galsserSomato_mask.nii',...
%         'space-MNI_label-glasserVip_mask.nii'};
    
    % use in output roi name
    opt.maskLabel = { 'lhMT','rhMT', 'lS1', 'lPC','rPC', 'lMTt', 'rMTt'};%,'M1','S1', 'PC'};
